...
export function parseRichText(document: EntryFields.RichText | Node | Document, composables?: { $img: any }, options?: Options) {
  return documentToHtmlString(document, {
    renderNode: {
      ...
      'hyperlink': (node) => {
        return `<a href="${node.data.uri}" ${node.data.uri.startsWith('/') || node.data.uri.includes('https://untoldjourneys.co.uk') ? '' : 'rel="nofollow noopener" target="_blank"'}>${node.content[0].value}</a>`
      },
